...
def load_dataset_pages(name, pages_count):
    ...
    return emnist_pages, kmnist_pages


def filter_white_characters(emnist_chars, kmnist_chars, threshold):
    filtered_emnist = []
    filtered_kmnist = []

    for emnist_char, kmnist_char in zip(emnist_chars, kmnist_chars):
    ...
    return np.array(filtered_emnist), np.array(filtered_kmnist)
